...
<div class="flex items-center justify-between mb-6">
    ...
    <form method="GET" action="{{ route('admin.orders.index') }}" class="flex flex-wrap items-center gap-2">
        <input type="text" name="q" value="{{ $search }}" placeholder="Search by user or status..."
            class="w-40 sm:w-64 px-3 py-2 rounded-lg bg-white border border-beige text-ink placeholder-gray-400 focus:border-indigo-500 focus:ring-indigo-500 shadow-soft">
    ...
    <table class="min-w-full divide-y divide-beige">
        ...
        <tbody class="divide-y divide-beige">
            ...
            <tr class="odd:bg-white even:bg-warm">
                ...
                <td class="px-4 py-3 text-sm text-ink">₫{{ number_format((float)$order->total_amount, 0, ',', '.') }}
                </td>
                <td class="px-4 py-3 text-sm">
                    ...
                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs {{ $badge[1] }}">{{ $badge[0]
                        }}</span>
                </td>
