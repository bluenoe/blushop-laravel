    ...
    <section class="max-w-7xl mx-auto px-6 py-12 sm:py-16">
        <div class="flex items-center justify-between">
            ...
            <form action="{{ route('wishlist.clear') }}" method="POST">
                @csrf
                <button type="submit"
                    class="rounded-lg bg-indigo-600 text-white font-semibold px-4 py-2 hover:bg-indigo-700">Clear
                    All</button>
            </form>
            @endif
        </div>
        ...
        @if(session('success'))
        <div class="mt-4 rounded-md border border-green-200 bg-green-50 text-green-700 px-4 py-2">{{ session('success')
            }}</div>
        @endif
