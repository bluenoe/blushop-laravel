<x-app-layout>
    <section class="max-w-7xl mx-auto px-6 py-12 sm:py-16" x-data="{ loading: true }"
        ...
        <div class="flex items-center justify-between" data-reveal="fade-up">
            ...
            <a href="{{ route('products.index') }}"
                class="rounded-md bg-indigo-600 text-white px-4 py-2 font-semibold hover:bg-indigo-500">Shop More</a>
        </div>

        @if (session('success'))
        <div x-data="{ show: true }" x-show="show" x-transition.opacity
            class="mt-4 rounded-md border border-green-200 bg-green-50 text-green-700 px-4 py-3">
            {{ session('success') }}
        </div>
        @endif
        ...
        <div class="mt-8 space-y-4">
            ...
            <div class="rounded-xl border border-beige bg-white shadow-soft overflow-hidden" x-data="{ open: false }">
                <div class="p-4 sm:p-6 flex items-center justify-between">
                    <div class="space-y-1">
                        <p class="text-sm text-gray-700">Order #{{ $order->order_code }} • {{
                            $order->created_at->format('M d, Y') }}</p>
                        <p class="text-lg font-semibold text-ink">₫{{ number_format((float)$order->total_amount, 0, ',',
                            '.') }}</p>
                    ...
                    <button type="button" @click="open = !open"
                        class="rounded-md border border-beige text-ink px-3 py-2 hover:bg-beige focus:outline-none focus:ring-2 focus:ring-indigo-500">
                        ...
                        <ul>
                            ...
                            <li class="p-4 sm:p-6 flex items-center gap-4">
                                ...
                                <div class="text-right">
                                    <p class="text-ink font-medium">₫{{ number_format((float)$item->price_at_purchase,
                                        0, ',', '.') }}</p>
                                </div>
                            </li>
                            @endforeach
