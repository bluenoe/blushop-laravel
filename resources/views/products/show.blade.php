...
<x-app-layout>
    ...
    <main class="bg-white text-neutral-900 selection:bg-neutral-900 selection:text-white">
        ...
        // Logic tạo ảnh: Ảnh 1 là ảnh thật, 3 ảnh sau tự động tìm theo tên Category

        $keyword = $product->category ? Str::slug($product->category->name) : 'minimalist-fashion';

        $gallery = [Storage::url('products/' . $product->image)];



        // Tạo thêm 3 ảnh random nhưng cố định theo ID sản phẩm (reload không bị đổi ảnh)

        for($i=1; $i<=3; $i++) { $gallery[]="https://loremflickr.com/1000/1200/{$keyword}?lock={$product->id}{$i}" ; }
            @endphp <div class="lg:grid lg:grid-cols-12 lg:gap-16 items-start" x-data="{
                            ...
                            <div class="flex items-start justify-between mb-4">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <span
                                            class="text-[10px] text-neutral-400 uppercase tracking-wider font-medium">{{
