    ...
    <section class="max-w-7xl mx-auto px-6 py-12 sm:py-16">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-900 transition">
                        ...
                        <td class="px-4 py-3 text-gray-900 dark:text-gray-100">₫{{ number_format((float)$item['price'],
                            0, ',', '.') }}</td>
                        <td class="px-4 py-3">
                            <form method="POST" action="{{ route('cart.update', $id) }}"
                                ...
                                <button
                                    class="inline-flex items-center rounded-md border border-indigo-600 text-indigo-600 px-3 py-1 text-sm hover:bg-indigo-50 dark:hover:bg-indigo-900 transition"
                        ...
                        <td class="px-4 py-3">
                            <form method="POST" action="{{ route('cart.remove', $id) }}">
                                @csrf
                                <button
                                    class="inline-flex items-center rounded-md border border-red-600 text-red-600 px-3 py-1 text-sm hover:bg-red-50 dark:hover:bg-red-900 transition"
                    ...
                    @endforeach
                    <tr>
                        <td colspan="4" class="px-4 py-3 text-right font-bold text-gray-900 dark:text-gray-100">Total
                        </td>
                        <td class="px-4 py-3 font-bold text-gray-900 dark:text-gray-100">₫{{ number_format($total, 0,
                            ',', '.') }}</td>
                        <td></td>
                    </tr>
                </tbody>
        ...
        <div class="mt-6 flex flex-wrap gap-3">
            <form method="POST" action="{{ route('cart.clear') }}">
                @csrf
                <button
                    class="inline-flex items-center rounded-md border border-gray-400 text-gray-700 px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-900 transition"
                    type="submit">Clear Cart</button>
            </form>
            <a href="{{ route('home') }}"
                class="inline-flex items-center rounded-md border border-gray-600 text-gray-800 dark:text-gray-200 px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-900 transition">Continue
                Shopping</a>
