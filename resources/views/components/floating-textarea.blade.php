...
<div class="relative z-0 w-full mb-6 group">
    <textarea {{ $disabled ? 'disabled' : '' }} name="{{ $name }}" id="{{ $name }}" placeholder=" " rows="3" {{
        $attributes->merge(['class' => 'block py-2.5 px-0 w-full text-sm bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 peer resize-none transition-colors duration-200 ' . 
            ($errors->has($name) 
    ...
    @error($name)
    <p class="mt-1 text-[10px] font-bold text-red-500 uppercase tracking-wider animate-pulse">{{ $message }}</p>
    @enderror
</div>