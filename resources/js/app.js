...
document.addEventListener("alpine:init", () => {
    ...
    Alpine.store("cart", {
        count:
            typeof window.__CART_COUNT === "number" ? window.__CART_COUNT : 0,
        set(n) {
            this.count = Number(n) || 0;
        },
        increment(n = 1) {
            this.count = this.count + (Number(n) || 0);
        },
    });
});
...
window.addEventListener("avatar:updated", (e) => {
    ...
    document
        .querySelectorAll('[data-avatar-placeholder="true"]')
        .forEach((ph) => {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!nav) return;
    const apply = () => {
        nav.classList.toggle("shadow-soft", window.scrollY > 0);
    };
    apply();
